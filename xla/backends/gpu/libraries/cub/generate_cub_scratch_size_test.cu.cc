...
template <typename KeyT>
absl::Status GenerateDataForKeyOnlySort(
...
absl::StatusOr<std::string> GetDeviceName() {
  int device = 0;
  cudaDeviceProp prop;
...
// Create a list of numbers that grows geometrically with the given
// growth factor, and at least 'min_step' apart.
std::vector<int64_t> CreateNumItemsList(int64_t max_items, int64_t min_step,
                                        double growth_factor) {
  std::vector<int64_t> num_items_list;
  for (int64_t n = 1; n <= max_items;
       n = std::max(n + min_step, static_cast<int64_t>(n * growth_factor))) {
...
TEST(ExtractCubScratchSizeTest, ExtractData) {
  ASSERT_OK_AND_ASSIGN(std::string device_name, GetDeviceName());

  // Create the set of values of number of items of the sorts that we'll
  // generate data for.
  constexpr int64_t kMaxItems = 2'000'000'000;
  constexpr int64_t kMinStep = 100;
  constexpr double kGrowthFactor = 1.02;  // 2% increase each time
  std::vector<int64_t> num_items_list =
      CreateNumItemsList(kMaxItems, kMinStep, kGrowthFactor);

  CubScratchSizeLookupTable lookup_table;
  // Generate the data for 8, 16, 32, 64 bit, key only sorts
  ASSERT_OK((GenerateDataForKeyOnlySort<int8_t>(lookup_table, device_name,
                                                num_items_list)));
  ASSERT_OK((GenerateDataForKeyOnlySort<int16_t>(lookup_table, device_name,
                                                 num_items_list)));
  ASSERT_OK((GenerateDataForKeyOnlySort<int32_t>(lookup_table, device_name,
                                                 num_items_list)));
  ASSERT_OK((GenerateDataForKeyOnlySort<int64_t>(lookup_table, device_name,
                                                 num_items_list)));
  ...
  // 8 bit keys
  ASSERT_OK((GenerateDataForKeyValueSort<int8_t, int8_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int8_t, int16_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int8_t, int32_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int8_t, int64_t>(
      lookup_table, device_name, num_items_list)));

  // 16 bit keys
  ASSERT_OK((GenerateDataForKeyValueSort<int16_t, int8_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int16_t, int16_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int16_t, int32_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int16_t, int64_t>(
      lookup_table, device_name, num_items_list)));

  // 32 bit keys
  ASSERT_OK((GenerateDataForKeyValueSort<int32_t, int8_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int32_t, int16_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int32_t, int32_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int32_t, int64_t>(
      lookup_table, device_name, num_items_list)));

  // 64 bit keys
  ASSERT_OK((GenerateDataForKeyValueSort<int64_t, int8_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int64_t, int16_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int64_t, int32_t>(
      lookup_table, device_name, num_items_list)));
  ASSERT_OK((GenerateDataForKeyValueSort<int64_t, int64_t>(
      lookup_table, device_name, num_items_list)));

  ASSERT_OK(WriteLookupTableToFile(lookup_table));
}
